...
(async () => {
 ...
 try {
  ...
  for(const letter of ['A','B','C']) {
   ...
   assert(!(await page.getByLabel('공통 공개 이미지 주소',{exact:true}).isVisible()));
   const copy=page.getByRole('button',{name:'HTML 코드 복사'});
   assert(await copy.isEnabled());
   await page.locator('summary').click();
   await page.getByLabel('공통 공개 이미지 주소',{exact:true}).fill('http://192.168.0.47:4173/test.png');
   assert(await copy.isDisabled());
   await page.getByLabel('공통 공개 이미지 주소',{exact:true}).fill('javascript:alert(1)');
   assert(await copy.isDisabled());
   await page.getByLabel('공통 공개 이미지 주소',{exact:true}).fill(`https://example.com/${letter}.png?v=2&lang=ko`);
   await page.getByRole('status').filter({hasText:'공고 이미지가 준비되었습니다.'}).waitFor();
   const code=await page.getByLabel('붙여 넣을 HTML').inputValue();
   assert(code.includes('&amp;lang=ko'));
   assert(!code.includes('<picture>'));
   assert.equal((code.match(/<img /g)||[]).length,1);
   ...
   for (const width of [360,390,720,1000]) {
    await rendered.setViewportSize({width,height:900});
    const box=await rendered.locator('img').boundingBox();
    assert(box.width<=720 && box.width<=width);
   }
   await rendered.close();
   assert.equal(await page.getByRole('link',{name:'공통 이미지 보기 ↗',exact:true}).getAttribute('href'),`https://example.com/${letter}.png?v=2&lang=ko`);
   await copy.click();
  ...
  await page.getByLabel('공통 공개 이미지 주소',{exact:true}).fill('https://example.com/missing.png');
