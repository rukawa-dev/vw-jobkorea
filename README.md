...
기본 표시 방식은 **PC 이미지 공통 사용 (잡코리아 권장)**입니다. 일반 `<img>`를 사용해 PC와 모바일에서 같은 이미지를 화면 너비에 맞춰 표시합니다. PC 이미지가 준비되면 복사할 수 있습니다. 저장 후 미리보기뿐 아니라 실제 공고 페이지에서도 표시 여부를 확인하세요.

**PC + MO 자동 전환 (잡코리아 비권장)**은 `<picture>`를 사용해 화면 너비 767px 이하에서는 MO 이미지로 전환합니다. 잡코리아 실제 공고에서 이미지 영역이 제거되는 사례를 확인했으며, 이 태그의 필터링이 원인으로 의심됩니다. 자동 전환은 해당 태그를 지원하는 게시 환경에서만 사용하세요. 이 모드에서는 PC·MO 이미지가 모두 준비되어야 복사할 수 있으며, 이미지 주소 변경에서 각각 지정할 수 있습니다.
