...
function Gallery({ onShowCode, planner = false }) {
  ...
  return <main className="gallery" id="content">
    ...
    <nav className="role-navigation" aria-label="채용 직무"><a href="#/" aria-current={!planner ? 'page' : undefined}>디자이너 채용</a><a href="#/planner" aria-current={planner ? 'page' : undefined}>기획자 · PM 채용</a></nav><section className="gallery-intro"><div><p className="overline">ONE OPPORTUNITY. THREE PERSPECTIVES.</p><h1>같은 채용,<br/>세 가지 첫인상<span>.</span></h1></div><div className="intro-aside"><span className="open-label"><i/> WE’RE HIRING</span><p>브랜드의 다음을 함께 고민할<br/><strong>{planner ? '디지털 기획자 · PM' : '디지털 · UI/UX 디자이너'}</strong>{planner ? '을' : '를'} 찾습니다.</p><p className="intro-meta">{planner ? '경력 2년 이상' : '신입 · 경력'} &nbsp; / &nbsp; 정규직 &nbsp; / &nbsp; 서울 마포</p></div></section>
...
function App() {
  ...
  useEffect(() => { document.title = design ? `VW — ${design.name} | ${role} 채용` : `VW — ${role} 채용 컬렉션`; }, [design, role]);
  const Page = design?.component;
  return <><a className="skip-link" href="#content" onClick={event => {event.preventDefault(); document.getElementById('content')?.scrollIntoView(); document.getElementById('content')?.focus();}}>본문으로 건너뛰기</a>{design ? <><nav className="design-nav" aria-label="시안 전환"><a className="back-link" href={home}>← <span>전체 시안</span></a><div>{collection.map(item => <a key={item.id} href={`#/${item.id}`} aria-current={route === item.id ? 'page' : undefined}>{item.letter ? `${item.letter}. ` : ''}{item.name}</a>)}</div><div className="nav-tools"><button className="html-code-button" onClick={() => setCodeDesign(design)}>채용공고 HTML 코드</button><button onClick={() => window.print()} aria-label="현재 채용공고 인쇄">인쇄 ↗</button></div></nav><Page/><div className="page-end"><a href={home}>← 전체 시안으로 돌아가기</a><button onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}>맨 위로 ↑</button></div></> : route && route !== 'planner' ? <main className="not-found"><h1>페이지를 찾을 수 없습니다.</h1><a href="#/">전체 시안으로 돌아가기 →</a></main> : <Gallery onShowCode={setCodeDesign} planner={planner}/>}{codeDesign && <HtmlCodeDialog key={codeDesign.id} design={codeDesign} onClose={() => setCodeDesign(null)}/>}</>;
}
