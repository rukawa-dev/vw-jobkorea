...
function App() {
  ...
  return <><a className="skip-link" href="#content" onClick={event => {event.preventDefault(); const content = document.getElementById('content'); if (content) { content.tabIndex = -1; content.focus({ preventScroll: true }); content.scrollIntoView(); }}}>본문으로 건너뛰기</a>{design ? <><nav className="design-nav" aria-label="시안 전환"><a className="back-link" href={home}>← <span>전체 시안</span></a><div>{collection.map(item => <a key={item.id} href={`#/${item.route}`} aria-current={design.id === item.id ? 'page' : undefined}>{item.letter ? `${item.letter}. ` : ''}{item.name}</a>)}</div><div className="nav-tools"><button className="html-code-button" onClick={() => setCodeDesign(design)}>채용공고 HTML 코드</button><a href={import.meta.env.BASE_URL + design.imagePath} download>공통 이미지 다운로드 ↓</a><button onClick={() => window.print()} aria-label="현재 채용공고 인쇄">인쇄 ↗</button></div></nav><div className="artwork-frame"><Page recruitment={recruitment} design={design}/></div><div className="page-end"><a href={home}>← 전체 시안으로 돌아가기</a><button onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}>맨 위로 ↑</button></div></> : !route ? <RecruitmentList/> : !recruitment ? <main className="not-found" id="content" tabIndex={-1}><h1>페이지를 찾을 수 없습니다.</h1><a href="#/">등록일별 공고 목록으로 돌아가기 →</a></main> : <Gallery onShowCode={setCodeDesign} recruitment={recruitment}/>}{codeDesign && <HtmlCodeDialog key={codeDesign.storageKey} design={codeDesign} onClose={() => setCodeDesign(null)}/>}</>;
}
